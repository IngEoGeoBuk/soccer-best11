...
import '@components/posts/common/playerBox.css';
...
function SelectedPlayerBox({ value }: { value: number }) {
  ...
  const t = useTranslations('post.playerBox');
  ...
  return (
    ...
    >
      ...
      <p className="player-box-name-paragraph">{t('name')}</p>
    </button>
  );
}
...
function ListPlayerBox({ player }: { player: Player }) {
  ...
  const clickFnc = () => {
    if (selectedCard === -1) {
      updateToastMessage(t('selectPosition'));
    ...
    if (selectedPlayers.includes(player)) {
      updateToastMessage(t('alreadyChosen'));
      document.getElementById('playerListSection')!.scrollIntoView();
      setTimeout(() => {
        updateToastMessage('');
      }, 2000);
